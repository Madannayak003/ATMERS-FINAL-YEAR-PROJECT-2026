...
def generate_pdf_report(c1, c2, c3, c4, emergency_lane=None):
    now = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    ...
    if emergency_lane:
        content.append(Paragraph(f"Emergency Detected in Lane {emergency_lane}", styles['Normal']))
...
def log_traffic_data(c1, c2, c3, c4, emergency_lane):
    now = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    ...
    log_line = (
        ...
        f"TOTAL:{total} | "
        f"EMERGENCY:{emergency_lane if emergency_lane else 'NO'}\n"
    )
...
while True:
    ...
    for ln, cnt, green in lanes:
        ...
        for i in [1, 2, 3, 6]:
            if i == ln:
                status.append(f"Lane {i} GREEN")
            else:
                status.append(f"Lane {i} RED")
